...
void ControlHelpSystem::RenderHelp(int _selectedType)
{
	...
	switch (_selectedType)
	{
		case Entity::TypeOfficer :
			...
			name = e && (g_app->m_userInput->GetMousePos3d()-e->m_pos).MagSquared() < 2500 ? (char*)"multiwinia_control_follow" : (char*)"multiwinia_control_order";
...
void ControlHelpSystem::RenderControlHelp( int position, const char *caption, const char *iconKb, const char *iconXbox, float alpha, float highlight, float yOffset, bool tutorialHighlight )
{
    float overallScale = 1.0f;

    bool keyboardInput = ( g_inputManager->getInputMode() == INPUT_MODE_KEYBOARD );
    const char *icon = ( keyboardInput ? iconKb : iconXbox );
    ...
    static const char *s_captions[]   = { NULL, NULL, NULL, NULL, NULL };
    static const char *s_icons[]      = { NULL, NULL, NULL, NULL, NULL };
