...
static bool CreateDirectoryRecursively(const char *_directory)
{
	const char *p;
	char *buffer;
	bool error = false;
	...
	while (p != NULL && !error)
	{
		...
		error = !CreateDirectory(buffer, NULL);
		p = strchr(p+1, '/');
	}
	...
	if (error)
		...
		return CreateDirectory(_directory, NULL);
}
...
BOOL CDlg::OnInitDialog()
{
	...
	if (theApp.m_pszProfile)
	{
		CreateDirectoryRecursively(theApp.m_pszProfile);
	}

	//CString message;
	//message = "Profile: '";
	//message += theApp.m_pszProfile;
	//message += "'.";
	//MessageBox(message, "debug", MB_OK );
...
void CDlg::GetFileDetails()
{
	...
	for (int n = 1; n <= MAX_INI_ITEMS; n++)
	{
		...
		{
			if (theApp.m_pszProfile)
				strFilePath = theApp.m_pszProfile;
			else
...
void CDlg::GetRegistryDetails()
{
	...
	CString strIniPath = m_szModulePath;
	if (theApp.m_pszProfile)
		strIniPath = theApp.m_pszProfile;
	strIniPath += INI_FILE_NAME;
...
int CDlg::ZipFiles()
{
	...
	if (theApp.m_pszProfile)
		m_strZipFile = theApp.m_pszProfile;
...
void CDlg::OnSend() 
{
	...
	if (!bRet)
	{
		...
		_sntprintf( szMsg, _countof(szMsg)-2,
					...
					_T("This reference is also recorded in errorlog.txt in your Multiwinia directory.\n\n")
					_T("Please use this reference when refering to the crash on the\n")
					_T("Multiwinia forums or in communication with Introversion. This will\n")
