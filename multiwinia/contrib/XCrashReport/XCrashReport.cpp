...
BOOL CApp::InitInstance()
{
	...
	if (__argc > 5)
	{
		m_pszProfile = __targv[5];

		// Decode '\\' as '\'
		TCHAR *pszProfileR = m_pszProfile;
		TCHAR *pszProfileW = m_pszProfile;
		BOOL lastBackSlash = 0;
		for( ; *pszProfileR; pszProfileR++)
		{
			if (*pszProfileR == '\\')
			{
				if (!lastBackSlash)
				{
					*(pszProfileW++) = *pszProfileR;
					lastBackSlash = 1;
				}
			}
			else
