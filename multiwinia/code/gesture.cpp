...
bool Gesture::CalculateFeatureSet()
{
    ...
    m_featureSet.f[1] = startDX / iv_sqrt( startDX * startDX + startDY * startDY );
    

    //
    // Compute f2 : sine of angle between sample[0] and sample[2]
    m_featureSet.f[2] = startDY / iv_sqrt( startDX * startDX + startDY * startDY );
    ...
    m_featureSet.f[3] = iv_sqrt( (boundR - boundL) * (boundR - boundL) + (boundB - boundT) * (boundB - boundT) );


    //
    // Compute f4 : angle of bounding box diagonal
    m_featureSet.f[4] = iv_atan( (boundB - boundT) / (boundR - boundL) );
    ...
    m_featureSet.f[5] = iv_sqrt( firstToLastDX * firstToLastDX + firstToLastDY * firstToLastDY );
    ...
    for( int j = 0; j < m_nextMouseSample-2; ++j )
    {
        ...
        totalGestureLength += iv_sqrt( dX * dX + dY * dY );
    }
    ...
    for( int k = 1; k < m_nextMouseSample-1; ++k )
    {
        ...
        sumAbsoluteAngles += iv_abs(angle);
...
void Gesture::AddTrainingSample( int symbolID )
{
    ...
    AppDebugAssert( newDataClass->m_nextTrainer < GESTURE_MAX_TRAINERS );
...
void Gesture::LoadTrainingData( char *filename )
{
    ...
    AppReleaseAssert(reader && reader->IsOpen(), "Couldn't open Gesture data file");
...
void Gesture::AddSample( int x, int y )
{
    ...
    if( !tooClose )
    {
        ...
        ++m_nextMouseSample;
        AppDebugAssert( m_nextMouseSample < GESTURE_MAX_MOUSE_SAMPLES );
    }
}
