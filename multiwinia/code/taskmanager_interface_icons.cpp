...
{   
    if( m_screenY != m_desiredScreenY )
    {
        ...
        if( iv_abs( m_desiredScreenY - m_screenY ) < 0.001f )
        {
...
void TaskManagerInterfaceIcons::AdvanceScreenEdges()
{
    ...
    bool scrollPermitted = iv_abs(m_desiredScreenY - m_screenY) == 0;
    ...
    if( stricmp( _name, "NewTask" ) == 0 )
    {
        ...
        if( g_app->m_globalWorld->m_research->HasResearch( _data ) )
        {
			if( _data == GlobalResearch::TypeSquad && g_app->m_globalWorld )
			{
				...
				AppDebugOut("Now have run %d squaddies for location index %d\n", 
...
void TaskManagerInterfaceIcons::RenderMessages()
{
    if( m_currentMessageType != -1 )
    {
		...
		if( !completedObjs && m_currentMessageType == MessageObjectivesComplete )
		{
			...
			AppDebugOut("Mission is completed - %f - %f\n", 
